...
本项目采用 **混合 + 离线兜底** 策略:默认对接 **智谱 GLM**（Anthropic 兼容端点）+ 可选 BGE 模型;当 **未配置** `ANTHROPIC_AUTH_TOKEN`（智谱 Bearer 令牌）时，自动降级为 **mock LLM / 哈希向量 embedding / 词法 reranker / 内存向量库**，因此 **无需任何密钥即可跑通全部 Demo 与测试**。而当 **已配置密钥但鉴权 / 额度 / 模型不可用** 时，平台会 **启动即校验** 并返回 **清晰的中文错误（HTTP 502）**，不再静默退回 mock 以免误导。
...
> 若配置了 `ANTHROPIC_AUTH_TOKEN`，启动日志会打印 `LLM 凭证校验通过/失败`，可据此第一时间发现密钥 / 模型问题。
...
```bash
ANTHROPIC_AUTH_TOKEN=<id.secret> MODEL_NAME=glm-4.5-air EMBEDDING_BACKEND=openai docker compose up --build
```
...
| `ANTHROPIC_AUTH_TOKEN` | 空 | **留空即离线 mock 模式**；智谱 Bearer 令牌（Anthropic SDK 以 `Authorization: Bearer` 发送），为 `id.secret` 形式直接填入即可（兼容旧名 `AUTH_TOKEN` / `OPENAI_API_KEY`） |
| `ANTHROPIC_BASE_URL` | `https://open.bigmodel.cn/api/anthropic` | 智谱 Anthropic 兼容端点（对话/生成） |
...
> 当配置的 Provider 调用失败（鉴权 / 额度 / 模型），`/api/chat`、`/api/upload` 等接口统一返回 **HTTP 502**，响应体形如 `{"detail":"LLM鉴权失败：… 请检查 backend/.env 中的 ANTHROPIC_AUTH_TOKEN …"}`。
